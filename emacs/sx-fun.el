...
(defun sx-set-frame-name ()
   (interactive "*")
   (let (r1 frame-name)
    (message "Frame : [a]fb [b]uild [c]ern [d]had [h]bb [m]cb [o]rca [p]xl")
    (setq r1 (read-char-exclusive))
    (cond 
     ((eq r1 ?a)   (setq frame-name "afb"))
     ((eq r1 ?b)   (setq frame-name "build"))
     ((eq r1 ?c)   (setq frame-name "cern"))
     ((eq r1 ?d)   (setq frame-name "dhad"))
     ((eq r1 ?h)   (setq frame-name "hbb"))
     ((eq r1 ?m)   (setq frame-name "mcb"))
     ((eq r1 ?o)   (setq frame-name "orca"))
     ((eq r1 ?p)   (setq frame-name "pxl"))
     (setq frame-name "run"))
    ))
